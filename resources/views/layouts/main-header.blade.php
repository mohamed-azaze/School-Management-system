        ...
        <li class="nav-item dropdown mr-30">
            ...
            <div class="dropdown-menu dropdown-menu-right">
                ...
                @if (auth('student')->check())
                    ...
                    <form id="logout-form" action="{{ route('logout.student') }}" method="POST"
                        style="display: none;">
                    @elseif (auth('teacher')->check())
                        <a class="dropdown-item" href="{{ route('logout.teacher') }}"
                            onclick="event.preventDefault();document.getElementById('logout-form').submit();"><i
                                class="text-danger ti-unlock"></i>{{ __('Sidebar_trans.Logoff') }}</a>
                        <form id="logout-form" action="{{ route('logout.teacher') }}" method="POST"
                            style="display: none;">
                        @elseif (auth('parent')->check())
                            <a class="dropdown-item" href="{{ route('logout.parent') }}"
                                onclick="event.preventDefault();document.getElementById('logout-form').submit();"><i
                                    class="text-danger ti-unlock"></i>{{ __('Sidebar_trans.Logoff') }}</a>
                            <form id="logout-form" action="{{ route('logout.parent') }}" method="POST"
                                style="display: none;">
